...
export class FlowtableCommand extends TableCommand {
    ...
    getWidths(lines) {
        widths = getColumnWidthsFromBorderSpec(lines);
	widths[widths.length-1] = widths[widths.length-1] + 1;
        return widths;
    }
}
...
const unifyTable = (table) => {
    ...
    for (let row of table) {
        ...
        if (curr_len < max_fields) {
            row = row.concat(new Array((max_fields - curr_len)).fill(''));
        }
        output.push(row);

        // mark columns that are not empty columns
        // (so the empties can be removed at the end)
